...
- Follow-up: require branch CI to pass before changing B0 from `In review` to `Verified`. Browser, GitHub connector, GitHub CLI, and API credentials were unavailable for automatic PR creation, so CI was expanded to every branch push; PR creation remains a manual review-boundary action.
