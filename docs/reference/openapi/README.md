...
./eng/test-openapi-lifecycle.ps1
./eng/verify-typescript-client.ps1
```

The first command intentionally updates the baseline and therefore belongs in a reviewed contract change. The second generates a candidate and a JSON diff report under ignored `artifacts/`; removed paths, operations, or schemas fail verification. Semantic fingerprints normalize JSON formatting and line endings before hashing. The current detector is deliberately conservative and does not yet classify every breaking schema mutation.

The lifecycle command creates disposable additive, deprecated, removed-path, removed-operation, and removed-schema fixtures. It proves accepted changes exit successfully and known breaking removals produce both a non-zero result and a machine-readable report.

The final command restores the repository-pinned Kiota tool, generates a TypeScript client into ignored build artifacts, installs exact proof dependencies, and compiles the result in strict mode. Generated sources are not frontend source code and are not committed. This keeps contract feasibility independently reproducible while the legacy frontend remains frozen.
