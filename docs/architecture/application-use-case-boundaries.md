...
`AssertionAppService` now retains only definition reads, run-history reads, and store purge coordination. Write, run, and audit-generation commands have explicit use-case boundaries, while their repositories remain the atomic owners of definition and history persistence.
